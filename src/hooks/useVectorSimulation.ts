...
export const getNodeColor         = (d: SimNode) => d.isInput ? "#e2e8f4" : CATEGORY_COLOR[d.category!] ?? "#6b7fa3";
export const getNodeRadius        = (d: SimNode) => d.isInput ? 10 : 7 + d.similarity * 10;
export const getNodeFillOpacity   = (d: SimNode) => d.isInput ? 1 : 0.25 + d.similarity * 0.75;
export const getNodeStrokeOpacity = (d: SimNode) => d.isInput ? 0.9 : 0.5 + d.similarity * 0.5;
export const getLabelOffset       = (d: SimNode) => d.isInput ? -14 : -(12 + d.similarity * 10);

export const getLinkStrokeWidth   = (d: SimLink) => Math.max(0.5, d.similarity * 4);
export const getLinkStroke        = (d: SimLink) => d.similarity > 0.15 ? "#00d4a8" : "#1e2d45";
export const getLinkStrokeOpacity = (d: SimLink) => d.similarity >= LINK_THRESHOLD ? 0.3 + d.similarity * 0.7 : 0;

// ── 데이터 빌더 ──────────────────────────────────────────────────────────────

export function buildSimData(cx: number, cy: number, sims: Record<string, number>) {
  const hasSims = Object.keys(sims).length > 0;
  const getSim  = (id: string) => hasSims ? (sims[id] ?? 0) : FALLBACK_SIMILARITY;

  const nodes: SimNode[] = [
    { id: "__input__", label: "Input", isInput: true, similarity: 1, x: cx, y: cy },
    ...KNOWLEDGE_BASE.map((item) => ({
      id: item.id, label: item.label, isInput: false,
      category: item.category, similarity: getSim(item.id), description: item.description,
    })),
  ];
  const links: SimLink[] = KNOWLEDGE_BASE.map((item) => ({
    source: "__input__", target: item.id, similarity: getSim(item.id),
  }));
...
export function useVectorSimulation(
  ...
  useEffect(() => {
    ...
    const init = () => {
      ...
      const cx = width / 2;
      const cy = height / 2;
      const { nodes, links } = buildSimData(cx, cy, useWorkflowStore.getState().similarities);
  ...
  useEffect(() => {
    ...
    nodeSelRef.current.select("circle").transition().duration(600)
      .attr("r",              getNodeRadius)
      .attr("fill-opacity",   getNodeFillOpacity)
      .attr("stroke-opacity", getNodeStrokeOpacity);

    nodeSelRef.current.select("text").transition().duration(600)
      .attr("dy",          getLabelOffset)
      .attr("fill-opacity", getNodeStrokeOpacity);

    linkSelRef.current.transition().duration(600)
      .attr("stroke-width",   getLinkStrokeWidth)
      .attr("stroke",         getLinkStroke)
      .attr("stroke-opacity", getLinkStrokeOpacity);
